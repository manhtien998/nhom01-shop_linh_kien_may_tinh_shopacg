<?php
/**
 * Welcome page of Easy Store Theme
 *
...
if ( ! class_exists( 'Easy_Store_Welcome' ) ) :

class Easy_Store_Welcome {
	...
	public function admin_menu() {
		...
		$page = add_theme_page( esc_html__( 'Welcome', 'easy-store' ), esc_html__( 'Welcome', 'easy-store' ), 'activate_plugins', 'easy-store-welcome', array( $this, 'welcome_screen' ) );
	}
	...
	public function about_theme_styles( $hook ) {
		...
		wp_enqueue_style( 'mt-theme-review-notice', get_template_directory_uri() . '/inc/welcome/theme-review-notice.css', array(), esc_attr( $easy_store_version ) );
		if( 'appearance_page_easy-store-welcome' != $hook && 'themes.php' != $hook ) {
			return;
		}

		wp_enqueue_style( 'welcome-theme-style', get_template_directory_uri() . '/inc/welcome/welcome.css', array(), $easy_store_version );
	}
...
?>
		<div id="mt-theme-message" class="updated easy-store-message">
			...
			<p><?php printf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$s welcome page %3$s.', 'easy-store' ), esc_html( $theme_name ), '<a href="' . esc_url( admin_url( 'themes.php?page=easy-store-welcome' ) ) . '">', '</a>' ); ?></p>
			<p><a class="button button-primary button-hero" href="<?php echo esc_url( admin_url( 'themes.php?page=easy-store-welcome' ) ); ?>"><?php printf( esc_html__( 'Get started with %1$s', 'easy-store' ), esc_html( $theme_name ) ); ?></a></p>
		</div>
<?php
	}
...
?>
		<div class="theme-info-wrapper">
			<div class="easy-store-theme-info">
				<h1> <?php printf( esc_html__( 'About %1$s', 'easy-store' ), $theme_name ); ?> </h1>
			...
			<p class="easy-store-actions">
				...
				<a href="<?php echo esc_url( apply_filters( 'easy_store_demo_url', 'http://demo.mysterythemes.com/easy-store/' ) ); ?>" class="button button-secondary docs" target="_blank"><?php esc_html_e( 'View Demo', 'easy-store' ); ?></a>
				...
				<a href="<?php echo esc_url( apply_filters( 'easy_store_rating_url', 'https://wordpress.org/support/theme/easy-store/reviews/?filter=5' ) ); ?>" class="button button-secondary docs" target="_blank"><?php esc_html_e( 'Rate this theme', 'easy-store' ); ?></a>

				<a href="<?php echo esc_url( apply_filters( 'easy_store_wp_tutorials', 'https://wpallresources.com/' ) ); ?>" class="button button-secondary docs" target="_blank"><?php esc_html_e( 'More Tutorials', 'easy-store' ); ?></a>
			</p>
			...
			<div class="changelog">
				<div class="under-the-hood two-col">
					...
					<div class="col">
						...
						<p><a href="<?php echo esc_url( 'https://mysterythemes.com/support/forum/themes/free-themes/' ); ?>" class="button button-secondary" target="_blank"><?php esc_html_e( 'Support', 'easy-store' ); ?></a></p>
					</div>
...
<?php
	...
	public function easy_store_theme_rating_notice() {
		...
		// Set the installed time in `easy_store_theme_installed_time` option table.
...
<?php
	...
	public function easy_store_ignore_theme_review_notice() {
		...
		if ( isset( $_GET['mt_easy_store_ignore_theme_review_notice'] ) && '0' == $_GET['mt_easy_store_ignore_theme_review_notice'] ) {
			add_user_meta( $user_id, 'easy_store_ignore_theme_review_notice', 'true', true );
		}
	...
	public function easy_store_ignore_theme_review_notice_partially() {
		...
		/* If user clicks to ignore the notice, add that to their user meta */
		if ( isset( $_GET['mt_easy_store_ignore_theme_review_notice_partially'] ) && '0' == $_GET['mt_easy_store_ignore_theme_review_notice_partially'] ) {
			update_user_meta( $user_id, 'mt_easy_store_ignore_theme_review_notice_partially', strtotime( '+ 7 days' ) );
		}
    ...
    public function easy_store_admin_footer_text( $text ) {
        ...
        if ( 'appearance_page_easy-store-welcome' == $screen->id ) {

        	$theme = wp_get_theme( get_template() );
			$theme_name = $theme->get( 'Name' );

            $text = sprintf( __( 'If you like <strong>%1$s</strong> please leave us a %2$s rating. A huge thank you from <strong>Mystery Themes</strong> in advance!', 'easy-store' ), esc_html( $theme_name ), '<a href="https://wordpress.org/support/theme/easy-store/reviews/?filter=5#new-post" class="theme-rating" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a>' );
